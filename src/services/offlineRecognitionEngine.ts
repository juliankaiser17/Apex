...
export class OfflineRecognitionEngine {
  ...
  /**
   * Matches visual features against the normalized local vehicle catalog.
   * For still captures (default), completely eliminates historical frame blending
   * and processes strictly the current image's evidence.
   */
  public matchVehicle(features: VisualFeatureVector, isStillCapture: boolean = true): OfflineRecognitionResult | null {
    if (isStillCapture) {
      // For still captures: wipe frame buffer so there is ZERO cross-scan carryover
      this.frameBuffer = [features];
    } else {
    ...
    const currentAspect = isStillCapture
      ? features.aspectRatio
      : this.frameBuffer.reduce((a, b) => a + b.aspectRatio, 0) / this.frameBuffer.length;
    const currentEdges = isStillCapture
      ? features.edgeDensity
      : this.frameBuffer.reduce((a, b) => a + b.edgeDensity, 0) / this.frameBuffer.length;
