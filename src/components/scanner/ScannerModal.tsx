...
import { runAiAuthenticityCheck } from '../../utils/authenticityPipeline';
...
export const ScannerModal: React.FC = () => {
  ...
  // Two-Stage Authenticity Verification & GPT-4o AI Vision Identification
  useEffect(() => {
    if (phase === 'analyzing') {
      ...
      const processAuthenticityAndVision = async () => {
        const exifData = {
          dateTimeOriginal: new Date().toISOString(),
          gps: { lat: 22.2950, lng: 114.1720, accuracy: 5.0 },
          software: 'iOS 17.4 Rear Camera Hardware',
          width: 1920,
          height: 1080,
          isUserUploadedFile: false
        };

        const authResult = await runAiAuthenticityCheck(capturedPhotoUrl || '', exifData);

        if (!authResult.passed) {
          setAuthenticityError(authResult.rejectionReason);
          setPhase('rejected');
          return;
        }
        ...
        if (aiResult.needs_better_angle) {
          setAngleInstruction(aiResult.angle_instruction || 'Please take a photo from front three-quarter angle.');
          setPhase('camera');
          return;
        }
        ...
        const newCard: CarCard = {
          ...
          rarity: rarityEngineResult.rarity,
          rarityScore: rarityEngineResult.rarityScore,
          ...
          modsDetected: aiResult.aftermarket_parts_detected.map(p => ({
            part: p.part_name,
          ...
          country: 'Hong Kong',
          xpEarned: 100,
          marketValueLowUsd: aiResult.estimated_market_value_usd_low,
          marketValueHighUsd: aiResult.estimated_market_value_usd_high,
          scanValidated: true,
